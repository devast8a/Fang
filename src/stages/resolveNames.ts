...
import { Context, Decl, Expr, Node, RefFieldId, RefGlobal, RefGlobalDecl, RefLocal, RootId, Tag, Type, TypeGet, unreachable } from '../nodes';
...
export const resolveNames = createVisitor(VisitChildren, VisitType, VisitRefDecl, (context, node, id, state, {first}) => {
    switch (node.tag) {
        ...
        case Tag.RefFieldName: {
            ...
            const type = Expr.getReturnType(context, target);
            const ref = Type.getMember(context, type, node.field);

            return new RefFieldId(node.target, ref);
        }
